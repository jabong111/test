...
import bsib.sequence.Sequencer;
        ...
        try {
            ...
            if (theme.getParentId() == 0) {
                ...
                rsGroup = stmtGroup.executeQuery(
                    "select max(GROUP_ID) from THEME_MESSAGE_BS");
                int maxGroupId = 0;
                if (rsGroup.next()) {
                ...
                "select max(ORDER_NO) from THEME_MESSAGE_BS "+
            ...
            if (theme.getOrderNo() > 0) {
                ...
                "update THEME_MESSAGE_BS set ORDER_NO = ORDER_NO + 1 "+
            ...
            // 새로운 글의 번호를 구한다.
            theme.setId(Sequencer.nextId(conn, "THEME_MESSAGE_BS"));
            // 글을 삽입한다.
            pstmtInsertMessage = conn.prepareStatement(
            "insert into THEME_MESSAGE_BS values (?,?,?,?,?,?,?,?,?,?,?)");
            ...
            "insert into THEME_CONTENT_BS values (?,?)");
	   ...
	   public void update(Theme theme) throws Exception {
		      ...
		      try {
		         conn = getConnection();
		            ...
		            pstmtUpdateMessage = conn.prepareStatement(
		                "update THEME_MESSAGE_BS set NAME=?,EMAIL=?,IMAGE=?,TITLE=? "+
		                "where THEME_MESSAGE_ID=?");
		            pstmtUpdateContent = conn.prepareStatement(
		                "update THEME_CONTENT_BS set CONTENT=? "+
		                "where THEME_MESSAGE_ID=?");
		    ...
		    public int count(List whereCond, Map valueMap)
		             throws Exception {
		                 ...
		                 try {
		                     ...
		                     query.append("select count(*) from THEME_MESSAGE_BS ");
		    ...
		    public List selectList(List  whereCond, Map valueMap, int startRow, int endRow) throws Exception {
		           ...
		           try {
		               ...
		               query.append("       from THEME_MESSAGE_BS ");
		               if (whereCond != null && whereCond.size() > 0) {
		                   query.append("where ");
		                   for (int i = 0 ; i < whereCond.size() ; i++) {
		                       query.append(whereCond.get(i));
		           ...
		           try {
		               ...
		               if (rsMessage.next()) {
		                   ...
		                   pstmtContent = conn.prepareStatement(
		                       "select CONTENT from THEME_CONTENT_BS "+
		                       "where THEME_MESSAGE_ID = ?");
		    ...
		    public void delete(int id) throws Exception {
		           ...
		           try {
		               ...
		               pstmtMessage = conn.prepareStatement(
		                   "delete from THEME_MESSAGE_BS where THEME_MESSAGE_ID = ?");
		               pstmtContent = conn.prepareStatement(
		                   "delete from THEME_CONTENT_BS where THEME_MESSAGE_ID = ?");
