...
<title>게시판</title>
<link href = "style.css" rel = "stylesheey" type = "text/css">
<script>
function writeSave() {
...
<table width = "500" border = "1" cellspacing = "0" bgcolor = <%=bodyback_c %> align = center></table>
	<form>
	<tr height = 30>
	...
	<td align=center width=125><%=article.getSubject() %></td>
	</tr>
	
	<tr>
	<td align = center width =125 bgcolor=<%=value_c %>>rmfsodyd</td>
	<td align=center width=125><%=article.getContent() %></td>
	</tr>
	<tr height=30>
	<td colspan = 4 bgcolor =<%=value_c %> align =right>
		<input type = button value = 글수정 onclick = "document.location.href='jpdateForm.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<tr bgcolor=<%=value_c %> align=center>
		...
		<tr>
		<td colspan=2><%=dbc.getCommnett()%><td>
		<%} %>
		</tr>
	</table>
	<table width=500 border=0 cellspacing=0 cellpadding=0 bgcolor=<%=bodyback_c %> align=center>
...
<%
	if(count>0) {
		...
		int startPage = (int)(cCurrentPage/5)*+1;
		int pageBlock = 5;
