...
<table width="100%" cellpadding=1 cellspacing=2>
	...
	<c:if test="${empty list }">
		<tr>
			<td bgcolor="#f0f0f0" colspan="4" align="center">등록된 이미지가 없습니다.
			</td>
		</tr>
	</c:if>
	<c:if test="${! empty list }">
		<c:forEach var="theme" items="${list }">
			<tr bgcolor="#f0f0f0">
				<td><c:if test="${! empty theme.image }">
						...
						%>
						<img src="/KH10/image/${theme.image}.small.jpg" width="50">
					</c:if></td>
				<td><a href="javascript:goView(${theme.id })">${theme.title }</a></td>
				<td>${theme.name }</td>
				<td><fm:formatDate value="${theme.register }"
...
<form name="search" action="list.jsp" method="post">
	<input type="checkbox" name="search_cond" value="title">제목 <input
		type="checkbox" name=search_cond value="name">이름 <input
		type="text" name="search_key" value="" size=10> <input
		type="submit" value="검색"> <input type="button" value="전체목록"
